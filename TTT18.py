...
TILESIZE = 100
WINDOWWIDTH = 540     # display window's width 
WINDOWHEIGHT = 540    # display window's hreight
FPS = 60
BLANK = None
...
def drawBoard(board, message):
    ...
    if message:
        textSurf, textRect = makeText(message, MESSAGECOLOR, BGCOLOR, 130, 70)
...
def main():
    ...
    pygame.display.set_caption('Kartik Parmar - Tic Tac Toe.')
    BASICFONT = pygame.font.Font('freesansbold.ttf', BASICFONTSIZE)
    NEW_SURF, NEW_RECT = makeText(' Player vs AI ', TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 205, WINDOWHEIGHT - 60)
    NEW_SURF2, NEW_RECT2 = makeText(' Multiplayer ', TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 530, WINDOWHEIGHT - 60)
    board = [BLANK] * 9
    game_over = False
    x_turn = True
    msg = "Tic Tac Toe Project"
    drawBoard(board, msg)
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  #Exit condition/command
                pygame.quit()
                exit()
